...
defmodule Sneakers23.Inventory do
  ...
  @doc """
    mark_product_released!-> ## обработка локальным генсервером (being_replicated: false)
    Replication.mark_product_released! ->
    broadcast!({:mark_product_released!, product_id}) -> ## бродкаст остальным нодам
    handle_info({:mark_product_released!, product_id}, state) (Replication.Sever) ->
    mark_product_released! ## обработка теперь уже удаленными генсерверами (being_replicated: true)
  """
